...
- A DA admin token at one of:
  - `<target-repo>/.hlx/.da-token.json` (preferred)
  - `~/.hlx/.da-token.json` (fallback)
  - File shape: `{ "access_token": "<jwt>", ... }`
  - If absent: Phase 5 fails early with an actionable error. The
    user is expected to fetch a token via the DA admin UI or CLI.
